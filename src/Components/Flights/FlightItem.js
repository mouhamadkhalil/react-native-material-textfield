...
const FlightItem = ({ item, price, index, selectFlight, sessionId, fareRules, isSummary }) => {
    var combo = null;
    if (item?.AirItinerary?.OriginDestinationCombinations && item?.AirItinerary?.OriginDestinationCombinations.length > 0)
        combo = item?.AirItinerary?.OriginDestinationCombinations[0];
    else
        combo = item;
    ...
    return (
        combo ?
            <View key={'flight-' + index} style={{ flex: 1, flexDirection: 'column', marginTop: 20, backgroundColor: (combo.Selected && !isSummary) ? R.colors.blue : 'white' }}>
                ...
                <View style={{ flexDirection: 'row', borderTopWidth: 1, borderColor: "#eee" }}>
                    ...
                    <View style={{ width: '50%', alignItems: 'center', justifyContent: "center", borderRightWidth: 1, borderColor: "#eee", padding: 15 }}>
                        <Text style={{ fontSize: 17.5, color: R.colors.green }}>
                            ${!isSummary? item?.AirItineraryPricingInfo?.ItinTotalFare?.TotalFare?.AmountPerFan : price}
                            /<Text style={{ fontSize: 12 }}>{translate('1FanRoundtrip')}</Text>
                        </Text>
                    </View>
                    ...
                    <View style={{ width: '50%', alignItems: 'center', justifyContent: "center", padding: 15 }}>
                        {!isSummary ?
                            <CheckBox title={translate('selectFlight')}
                                ...
                                containerStyle={{ backgroundColor: (combo.Selected && !isSummary) ? R.colors.blue : 'white', borderWidth: 0 }}
                                checkedColor='white'
                        ...
                        <View style={{ flexDirection: "row", alignItems: "center", marginTop: 15 }}>
                            <View style={{ flexDirection: "row", alignItems: "center", width: 80, paddingRight: 7 }}>
                                {combo?.firstWay?.FlightSegment[0]?.FlightDuration ?
                                    ...
                                    : null}
                                <Image source={R.images.connectionIcon} style={{ width: 10, height: 30 }} />
                            </View>
                            <View style={{ flex: 1 }}>
                                <Text style={{ width: "80%" }}>
